...
def task_status(task_id):
    ...
    if result.get('status') == 'completed':
        ...
        elif isinstance(result.get('result'), dict) and 'success' in str(result.get('result')).lower() and 'true' in str(result.get('result')).lower():
        ...
        for d in dirs:
            ...
            try:
                ...
                if not os.listdir(dir_path):
                    ...
                    dir_modified = datetime.fromtimestamp(os.path.getmtime(dir_path))
                    if dir_modified < cutoff and not any(os.scandir(dir_path)):
                        os.rmdir(dir_path)
                        deleted_dirs += 1
            except Exception as e:
                error_count += 1
                app_logger.error(f"清理目录失败: {dir_path}, 错误: {str(e)}")

    app_logger.info(f"清理完成: 删除了 {deleted_files} 个文件, {deleted_dirs} 个目录, {error_count} 个错误")
